...
// Assertion style and config || Estilo de aserción y configuración
chai.should();
chai.use(chaiHttp);
...
describe("Test routes for REST-API", () => {
  ...
  /**
   * Test the GET route || Probar la ruta GET"/"
   */
  describe("GET /", () => {
  ...
  /**
   * Test the POST route || Probar la ruta POST "/"
   */
  describe("POST /", () => {
    it("Wrong POST body and path params should get a 404 error and message", (done) => {
      chai
        .request(server)
        .post("/developers/cc/0") // Params between body and path DON'T match || Los parámetros entre el cuerpo y la ruta NO coinciden
  ...
  /**
   * Test the PUT route || Probar la ruta PUT"/"
   */
  describe("PUT /", () => {
    it("Wrong PUT body and path params should get a 404 error and message", (done) => {
      chai
        .request(server)
        .put("/developers/cc/0") // Params between body and path DON'T match || Los parámetros entre el cuerpo y la ruta NO coinciden
        .set("content-type", "application/json")
        .send(fakeDeveloper1)
        .end((err, response) => {
          response.should.have.status(404);
  ...
  /**
   * Test the DELETE route || Probar la ruta DELETE"/"
   */
  describe("DELETE /", () => {
